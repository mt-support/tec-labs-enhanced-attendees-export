...
* `tec-enhanced-attendees-export` - Plugin Human-readable name, e.g. "Sample Extension"
* `Tecenhancedattendeesexport` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `tec-enhanced-attendees-export` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `tec_enhanced_attendees_export` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
